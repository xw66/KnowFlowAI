...
function Answer([long]$KnowledgeBaseId, [hashtable]$Body) {
    for ($attempt = 0; $attempt -lt 30; $attempt++) {
        $response = Invoke-WebRequest "$BaseUrl/api/knowledge-bases/$KnowledgeBaseId/answers" -Method Post -Headers $script:apiHeaders -ContentType 'application/json' -Body ($Body | ConvertTo-Json -Compress) -SkipHttpErrorCheck
        if ($response.StatusCode -eq 200) { return @{ status = 200; data = $response.Content | ConvertFrom-Json } }
        if ($response.StatusCode -ne 429) { return @{ status = [int]$response.StatusCode; data = $null } }
        $wait = 1; if ($response.Headers['Retry-After']) { [int]::TryParse([string]$response.Headers['Retry-After'], [ref]$wait) | Out-Null }
        Start-Sleep -Seconds ([Math]::Max(1, $wait))
    }
...
try {
    ...
    if ($EvaluateQa) {
        foreach ($sample in @($dataset.qa)) {
            $call = Answer $base.id @{ question = $sample.question; topK = 5; mode = 'HYBRID'; rerank = $false }
            $citations = if ($call.data) { @($call.data.citations) } else { @() }
            $citedNames = @($citations | ForEach-Object { [string]$_.documentName })
            $expectedNames = @($sample.citations | ForEach-Object { $expectedId = $_; [string](($dataset.documents | Where-Object { $_.id -eq $expectedId }).name) })
            $coverage = @($expectedNames | Where-Object { $citedNames -contains $_ }).Count -gt 0
            $exact = $citations.Count -gt 0 -and @($citedNames | Where-Object { $expectedNames -notcontains $_ }).Count -eq 0
            $qaRows.Add([pscustomobject]@{ id = $sample.id; question = $sample.question; httpStatus = $call.status; answerStatus = if ($call.data) { [string]$call.data.status } else { 'FAILED' }; citationCoverage = [int]$coverage; citationsAllExpected = [int]$exact; citationCount = $citations.Count; expectedDocuments = $expectedNames; citedDocuments = $citedNames; usage = if ($call.data) { $call.data.usage } else { $null }; answer = if ($call.data) { $call.data.answer } else { $null }; citations = $citations })
        }
    }
    ...
    $report = [pscustomobject]@{ generatedAt = [DateTime]::UtcNow.ToString('o'); dataset = $DatasetPath; knowledgeBaseId = $base.id; documentCount = $dataset.documents.Count; queryCount = $dataset.queries.Count; qaCount = $dataset.qa.Count; rerankRequested = [bool]$EvaluateRerank; summary = $summary; results = $rows; qaResults = $qaRows }
    ...
    if ($EvaluateQa) { $qaRows | Select-Object id,httpStatus,answerStatus,citationCoverage,citationsAllExpected,citationCount | Format-Table | Out-String | Write-Output }
