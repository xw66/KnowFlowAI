param([string]$BaseUrl='http://localhost:8080',[int]$WaitSeconds=180)
...
$report=[pscustomobject]@{generatedAt=(Get-Date).ToUniversalTime().ToString('o');dataset='docs/demo.md / 1 query';knowledgeBaseId=$base.id;results=$rows;rerank='NOT_EVALUATED'}
