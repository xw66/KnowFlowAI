...
4. 「成员与设置」修改名称或授权。VIEWER 不显示写入口；系统管理员和 OWNER 可管理开放范围与成员。用户 ID 在侧栏底部查看。
5. 「我的部门」支持加入、退出多个部门；管理员可创建部门。创建知识库及设置中可选择仅指定成员、全员只读或指定部门只读。规则详见 [部门与权限](../docs/departments.md)。
