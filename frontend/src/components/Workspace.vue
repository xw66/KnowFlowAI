...
import DepartmentsPane from './DepartmentsPane.vue'
...
const tab = shallowRef<'documents' | 'assistant' | 'settings' | 'departments'>('documents')
const more = shallowRef(false)
const creating = shallowRef(false)
const newName = shallowRef('')
const newSharing = shallowRef<Sharing>({ visibility: 'PRIVATE', departmentIds: [] })
...
function create() {
  void run(async () => {
    const base = await api.json<KnowledgeBase>('/knowledge-bases', { method: 'POST', body: JSON.stringify({ name: newName.value.trim(), ...newSharing.value }), signal })
    signal.throwIfAborted(); bases.value = [...bases.value, base]; select(base); creating.value = false; newName.value = ''
  })
}
function openCreate() {
  newSharing.value = { visibility: 'PRIVATE', departmentIds: [] }
  creating.value = true
  void run(async () => { departments.value = await loadDepartments(api, signal) })
}
function departmentsChanged() {
  bases.value = []; selectedId.value = undefined
  void loadBases().catch(cause => { if (!signal.aborted) error.value = errorText(cause) })
}
...
<template>
  <div class="workspace-shell">
    <aside class="sidebar" :class="{ open: menuOpen }">
      ...
      <div class="sidebar-heading"><span>我的知识库</span><button class="icon-button" aria-label="创建知识库" @click="openCreate">＋</button></div>
      ...
      <button class="sidebar-create" @click="openCreate">＋ 新建知识库</button>
      <button class="sidebar-create" @click="tab = 'departments'; menuOpen = false">用户组</button><div class="sidebar-footer"><div class="avatar">{{ user?.username.slice(0, 1).toUpperCase() ?? '·' }}</div><div class="account-name truncate">{{ user?.username ?? '正在加载' }}<small>用户 ID {{ user?.id ?? '—' }}</small></div><button class="icon-button" aria-label="退出登录" title="退出登录" @click="emit('logout')">↪</button></div>
    </aside>
    <main class="main-workspace">
      ...
      <DepartmentsPane v-if="tab === 'departments'" :api="api" :admin="user?.role === 'ADMIN'" @changed="departmentsChanged" />
      ...
      <div v-else class="welcome empty-state"><span class="empty-symbol" aria-hidden="true">▤</span><h1>{{ busy ? '正在加载知识库…' : '为团队的知识留一个位置' }}</h1><p>把文档放在一起，让每一个问题都有出处。</p><button v-if="!busy" class="primary" @click="openCreate">创建第一个知识库</button></div>
    </main>
    <ModalDialog v-if="creating" title="创建知识库" :busy="busy" @close="creating = false"><h2 id="create-title">创建知识库</h2><p class="muted">按团队、项目或主题组织资料，并选择开放范围。</p><form @submit.prevent="create"><label>知识库名称<input v-model="newName" autofocus required maxlength="128" placeholder="例如：产品研发手册" /></label><SharingFields v-model="newSharing" :departments="departments" :disabled="busy" /><p v-if="error" class="error" role="alert">{{ error }}</p><div class="modal-actions"><button type="button" :disabled="busy" @click="creating = false">取消</button><button class="primary" :disabled="busy || !newName.trim() || (newSharing.visibility === 'DEPARTMENTS' && !newSharing.departmentIds.length)">{{ busy ? '创建中…' : '创建' }}</button></div></form></ModalDialog>
  </div>
</template>
